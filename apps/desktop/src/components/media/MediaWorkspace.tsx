...
export function MediaWorkspace({ bundle, onLog, onNotify }: MediaWorkspaceProps) {
  ...
  const replaceCamera=useCallback((item:EdlManifest["tracks"]["camera"][number],conflictIds:string[])=>{
    ...
    commitEdl({...edl,tracks:{...edl.tracks,camera:edl.tracks.camera.filter(value=>value.id===item.id||!conflicts.has(value.id)).map(value=>value.id===item.id?item:value)}});
  },[commitEdl,edl]);
